...
class llm4rec(nn.Module):
    def __init__(
        ...
        if llm_model == 'opt':
            self.llm_model = OPTForCausalLM.from_pretrained("models/opt-6.7b", torch_dtype=torch.float16, use_safetensors=True).to("cuda")
            self.llm_tokenizer = AutoTokenizer.from_pretrained("models/opt-6.7b")
    ...
    def forward(self, log_emb, samples):
        ...
        text_output_tokens = self.llm_tokenizer(
            [t if isinstance(t, str) else " ".join(t) + self.llm_tokenizer.eos_token for t in samples['text_output']],
            return_tensors="pt",
            padding="longest",
            truncation=False,
        ).to(self.device)
