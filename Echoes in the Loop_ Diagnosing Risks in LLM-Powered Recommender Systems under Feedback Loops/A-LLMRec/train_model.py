...
def inference(user_id, user_train, args):
    print('A-LLMRec start inference\n')
    if args.multi_gpu:
        world_size = torch.cuda.device_count()
        mp.spawn(inference_ddp, args=(world_size, args), nprocs=world_size)
    else:
                             
        return user_inference(user_id, user_train, args)
...
def train_model_phase1_(rank, world_size, args):
    ...
    dataset = data_partition(args.rec_pre_trained_data, path=args.train_txt_path)
...
def train_model_phase2_(rank,world_size,args):
    ...
    model.train()
    t0 = time.time()
    for epoch in tqdm(range(epoch_start_idx, 6)):
        if args.multi_gpu:
            train_data_loader.sampler.set_epoch(epoch)
        for step, data in enumerate(train_data_loader):
            u, seq, pos, neg = data
...
def inference_ddp(rank, world_size, args):
    if args.multi_gpu:
        setup_ddp(rank, world_size)
        args.device = 'cuda:' + str(rank)
    ...
    user_train = defaultdict(list, user_train)
    user_test = defaultdict(list, user_test)
    ...
    open('A-LLMRec/recommendation_output.txt', 'w').close()                   
    for _, data in tqdm(enumerate(inference_data_loader), total=len(inference_data_loader), desc="Generating predictions"):
        u, seq, pos, neg = data
        u, seq, pos, neg = u.numpy(), seq.numpy(), pos.numpy(), neg.numpy()
        model([u,seq,pos,neg, rank], mode='generate')
    
def user_inference(user_id, pos_id, user_train, user_gt, args, model, seed):
    user_tr = {}
    user_va = {}
    user_te = {}
    user_list = [user_id]
    ...
    if len(user_train)>=3:
        user_tr[user_id] = user_train[:-2]
        user_va[user_id] = [user_train[-2]]
                                             
        user_te[user_id] = [pos_id]
    ...
    user_tr = defaultdict(list, user_tr)
    user_va = defaultdict(list, user_va)
    user_te = defaultdict(list, user_te)

    inference_data_set = SeqDataset_Inference(user_tr, user_va, user_te, user_list, args.item_num, args.maxlen)
    inference_data_loader = DataLoader(inference_data_set, batch_size=1, pin_memory=True)
    
    data = next(iter(inference_data_loader))
    u, seq, pos, neg = [x.numpy() for x in data]
    
    model.eval()
    
    predicted_title, candidates_include_target, candidates_include_target_titles = model([u,seq,pos,neg, user_gt, 0], mode='generate', seed=seed)
