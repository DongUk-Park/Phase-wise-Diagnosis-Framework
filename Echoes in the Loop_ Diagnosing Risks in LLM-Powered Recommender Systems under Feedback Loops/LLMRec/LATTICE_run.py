...
from LATTICE_Models import LATTICE
...
class Trainer(object):
    def __init__(self, data_config, args):
        ...
        self.args = args
        self.path = "LLMRec/LLMRec_c/movielens/ml100k_llmrec_format"

        self.model_name = "lattice"
        self.mess_dropout = eval(self.args.mess_dropout)
        self.lr = self.args.lr
        self.emb_dim = self.args.embed_size
        ...
        image_feats = np.load(self.path + '/image_feat.npy'.format(self.args.dataset))
        text_feats = np.load(self.path + '/text_feat.npy'.format(self.args.dataset))
    ...
    def test(self, users_to_test, is_val):
        ...
        result = test_torch(ua_embeddings, ia_embeddings, users_to_test, is_val, self.args)
        
        _, candidate_indices = torch.topk(torch.matmul(ua_embeddings, ia_embeddings.T), k=10, dim=1)
        save_path = os.path.join(self.args.data_path, self.args.dataset, 'candidate_indices')
...
def main():
    ...
    trainer = Trainer(data_config=config, args = args)
